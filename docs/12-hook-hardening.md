...
MODEL_PRICES = {
    ...
    "deepseek-chat": {"input": 0.30, "output": 0.60},
...
**token 来源**：适配层粗估（`max(1, len(text) * 2 // 3)`）。与 v2 的 `token_counter.py`（DeepSeek 官方 tokenizer）互补——粗估用于围栏决策（±20% 不影响"预算用完没"的判断），精确计数走 Langfuse 成本面板做事后分析。
...
**问题**（Architecture Agent）：CostGuard 使用 `max(1, len(text) * 2 // 3)` 粗估 token，而 v2 已有三级 tokenizer（DeepSeek 官方 > HuggingFace > rough）。
...
| **L3：E2E 级** | 真实 LLM + Hook 全链路 | 真实 DeepSeek | E2E-F-* (本文档) |
...
```python
async def llm_assert(reply: str, assertion: str, model: str = "deepseek-chat") -> bool:
