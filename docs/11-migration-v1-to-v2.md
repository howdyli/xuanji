...
v2 `memory/token_counter.py` 优先使用 DeepSeek 官方 tokenizer，但不同版本 dashscope SDK 支持情况不一，必须先校准。
...
- 次选：HuggingFace `DeepSeek/DeepSeek2-7B` tokenizer → 本地加载
...
| Token 计数 | 内联 `len//2` | `xiaopaw/memory/token_counter.py` | 三级降级：DeepSeek 官方 tokenizer → HuggingFace `DeepSeek/DeepSeek2-7B` → `len // 2`（v2.1：三值 Literal） | `token_counter_mode` (F1) |
...
□ 断 DeepSeek API：tenacity 重试后 503 返回；canary 存活
...
```bash
# Step 1: 恢复旧凭证（飞书 App / DeepSeek key）
# Step 2: 从归档恢复 v1 代码
...
### Q8: v2 的 DeepSeek 官方 tokenizer 找不到怎么办？
...
| 资源 | v1 | v2 | 差异 |
|---|---|---|---|
| 容器内存（稳态） | ~400MB | ~550MB | +150MB（DeepSeek tokenizer + LRUCache + trace） |
