...
边界 B3：Business 层 → Trusted / External 层
  数据流：
    ...
    F9  AliyunLLM → DeepSeek DashScope API
...
- 定时任务被重复创建（Cron 路径）
- 若消息触发外部 API 调用（飞书发消息 / DeepSeek），费用重复
...
开发过程中将真实凭证（DeepSeek API Key、飞书 App Secret、DB 密码等）硬编码进 `config.yaml` 并 git push；或测试代码中包含明文凭证；或 docker 镜像构建时将 `.env` 文件复制进层。
...
- DeepSeek API Key 泄露 → 攻击者消耗 quota 或访问模型
...
攻击者通过飞书消息以高频率向 Bot 发送消息，耗尽 Agent 处理队列或触发大量 DeepSeek API 调用，导致合法用户消息无法被处理，或造成 API 费用爆增。
...
- v1 无任何入站速率限制
- Agent 每次调用 DeepSeek API（成本 ~0.04¥/1k token），1000 次请求 = 数百元
- **可行性：HIGH（无防护时）**

**影响**

- 服务降级：合法用户排队超时
- 费用爆炸：DeepSeek API / 百度 API 费用无上限增长
...
networks:
  xiaopaw-net:
    driver: bridge
    internal: false   # XiaoPaw 主进程需要出公网调 DeepSeek API，网络非 internal-only
...
| DeepSeek API Key | 90 天 | 费用异常 / 安全事件 |
...
networks:
  xiaopaw-net:
    driver: bridge
    # internal: false —— XiaoPaw 主进程需要出公网调 DeepSeek / 百度 API
...
| 数据 | 接收方 | 数据类型 | PIPL 要求 |
|------|--------|---------|----------|
| 用户消息文本 | 阿里云 DashScope（DeepSeek API） | 可能含 PII | PIPL §23 委托处理 → 必须签 DPA；若 API 节点在境外 → PIPL §38-39 跨境传输评估 |
| 搜索查询 | 百度千帆 | 可能含 PII | 同上 |
| 记忆摘要 + 嵌入 | 阿里云 DashScope（embedding API） | 脱敏后，仍可能含语义 PII | 同上 |

**关键提醒（v2.1 补）**：
- DeepSeek DashScope 和百度千帆是"企业外部第三方"——即便它们是中国境内云，数据离开本企业边界就是外发
...
- 阿里云 DashScope（DeepSeek 推理 + embedding）
- 百度千帆（搜索）
...
- 跨境传输限制（Chapter V）→ DeepSeek/百度 API 需评估标准合同条款（SCC）
